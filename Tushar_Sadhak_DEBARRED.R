...
# 4c: Prime checker
# FIX: guard inner loop so it doesn't run backwards for small primes (e.g. n=3)
# Bug: `for (i in 3:floor(sqrt(3)))` = `3:1` → iterates 3,2,1 → wrongly returns FALSE
is_prime <- function(n) {
  if (n < 2)  return(FALSE)
  if (n == 2) return(TRUE)
  if (n %% 2 == 0) return(FALSE)
  upper <- floor(sqrt(n))                          # FIX: compute upper bound first
  if (upper >= 3) {                                # FIX: only loop if range is valid
    for (i in 3:upper) {
